...
> **Corroborated against the binaries.** These knob names come from the prompt
> logs — a record of what was *asked for*. The `.endl` files beside them record
> what was *built*, and `scripts/endl_analyze.py` extracts the knob-name strings
> they actually ship. **42 of the 69 names below appear verbatim inside the
> corresponding binary**, and every patch except OILY BOY (whose `D2`/`D3` are
> too short for the scan's 3-character floor) has at least one match. The two
> sources were produced independently, so where they agree the spec can be
> trusted. See [`docs/endl-corpus-study.md`](../../../docs/endl-corpus-study.md).
>
> The 27 non-matches are not contradictions: the NUL-delimited string scan is
> deliberately conservative and misses any literal packed directly after another
> string. No name recovered from a binary *disagreed* with this catalog.
