...
There are two independent sources, and they corroborate each other.

**The prompt log.** Each SpiralCaster example includes a `.pdf`. The final `(result)` block
is the most useful section — it describes the implemented algorithm in one paragraph with
parameter ranges and control behavior. This is the spec you're implementing, and
[`CATALOG.md`](../playground/examples/SpiralCaster_Examples/CATALOG.md) has it already
extracted for all 23.

**The binary itself.** When this guide was first written, the `.endl` was opaque. It is not:

```bash
python3 scripts/endl_inspect.py --entries --vtable "playground/examples/.../BELLOWS.endl"
python3 scripts/endl_analyze.py
```

That gives you the ABI entry points, the C++ vtable, the real `processAudio` address, and
the knob-name strings the patch actually ships. See
[`docs/reverse-engineering/`](reverse-engineering/) for how the audio path is recovered and
[`docs/endl-corpus-study.md`](endl-corpus-study.md) for what the whole corpus shows.

Use both. 42 of the 69 knob names `CATALOG.md` took from the PDFs appear verbatim as strings
inside the corresponding binaries — a prompt log describes intent, the binary shows what was
built, and where they agree you can trust the spec.
...
See [`source/Patch.h`](../source/Patch.h)'s `Patch::Color` enum for all 16 values —
the integer index is what crosses the ABI, so the declaration order matters.
...
public:
    ...
    const char* getParameterName(int idx) override {
        switch (idx) {
            case 0: return "SizeDec";
            case 1: return "Thresh";
            case 2: return "Tone";
...
See [`docs/fork-comparisons/upstream-and-forks.md`](fork-comparisons/upstream-and-forks.md)
for the fork survey, and `docs/endless-reference.md` §12 for ecosystem context.
...
- [`docs/reverse-engineering/`](reverse-engineering/) — recovering the audio path from a compiled `.endl`
- [`docs/endl-corpus-study.md`](endl-corpus-study.md) — what all 42 binaries in this repo show
