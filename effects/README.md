...
- `funk_machine_envelope_filter.cpp`: Mu-Tron-style touch envelope filter for bass, clav, clean guitar, and keyboards — linked-stereo envelope-driven Chamberlin SVF bandpass, with a 4-state Bass/GuitarKeys × Down/Up hold cycle
...
| `funk_machine_envelope_filter.cpp` | `Sensitivity`: how strongly playing dynamics move the filter — nonlinear so it stays useful across hot bass/keys and quieter guitar. | `Resonance`: broad/fat to sharp/vocal (Q 1.2–7.5). | `Bias`: shifts the whole envelope sweep window up or down; expression mapped. | Bypass toggle. | Advances a 4-state cycle: Bass-Down (default) → GuitarKeys-Down → GuitarKeys-Up → Bass-Up → loop. Voice flips on odd presses, direction flips on even presses — the first hold always reproduces the plain Bass↔GuitarKeys toggle. `Down` = louder opens the filter (original behavior); `Up` = louder closes it (Mu-Tron-III-style inversion). | Bass-Down: LightGreen active, DarkLime bypassed. GuitarKeys-Down: LightBlueColor active, DimCyan bypassed. GuitarKeys-Up: DarkCobalt active, DimCyan bypassed. Bass-Up: PastelGreen active, DimGreen bypassed. |
